...
class AsteroidSpawner extends PositionComponent with HasGameRef {
  ...
  FutureOr<void> onLoad() {
    ...
    spawner = SpawnComponent(
      factory: (idx) { //factory = pabrik, properti yang digunakan untuk membuat spawn baru ( asteroid )
        return Asteroid(ukuran: r.nextDouble() * 0.5 + 0.3);
      },
      period: 0.5,
      autoStart:
          true, //jarak penambahan spawn komponen baru, dalam satuan detik
      area: Rectangle.fromLTWH(0, 0, game.size.x, game.size.y),
      within:
          false, //asteroid hanya berada pada pinggir, tidak ada yang ditengah
    );
